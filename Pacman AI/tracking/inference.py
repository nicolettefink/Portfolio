    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.legalPositions:
                ...
                probNoisyDistance = emissionModel[trueDistance]
                probGhostPos = self.beliefs[p]
                allPossible[p] = probNoisyDistance * probGhostPos
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        #add each legal position to the particles list
        #keep going through the list and adding all legal
        #positions until you reach numParticles in
        #the particles list
        particles = []
        i = 0
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            for _ in range(0,self.numParticles):
                newParticles = newParticles + [self.getJailPosition()]
            self.particles = newParticles
        else:
            for particle in self.particles:
                trueDistance = util.manhattanDistance(particle,pacmanPosition)
                probNoisyDistance = emissionModel[trueDistance]
                allPossible[particle] = allPossible[particle] + probNoisyDistance
            ...
            """
            ### This is wrong, iterates over all legal positions.
            flag = False
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                probNoisyDistance = emissionModel[trueDistance]
                ...
                for pos in self.particles:
                    if pos == p:
                        counter = counter + 1
                        flag = True
                probGhostPos = counter / self.numParticles
                allPossible[p] = allPossible[p] + probNoisyDistance * probGhostPos
            if (flag): #any(allPossible.values()):
                newParticles = []
                for _ in range(0,self.numParticles): 
                    newParticles.append(util.sample(allPossible))
                self.particles = newParticles
